<template>
  <div class="city">
    ...
    <dl>
      ...
      <dd>
        <ul>
          <v-touch tag='li' @tap='changCity({cityId: item.id, n: item.n})' v-for="(item,index) in warmCity" :key="index">
            <a href="#">{{item.n}}</a>
          </v-touch>
        </ul>
      </dd>
    ...
    <div class="city" v-for="(item,index) in cityList" :key="index">
      <p>{{item.index}}</p>
      <ul>
        <v-touch v-for="(data,idx) in item.list" :key="idx" tag="li" @tap="handleToggleCity(data)">
          <a href="#">{{data.n}}</a>
        </v-touch>
      </ul>
    </div>
